...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function calculateStats() {
    ...
    if (filteredData.length === 0) {
      statsResults.innerHTML = `No data available for ${getMonthName(
        selectedMonth
      )}/${selectedYear}.`;
      return;
    }
    ...
    let totalMonthScore = filteredData.reduce(
    ...
    if (cumulativeData.length === 0) {
      statsResults.innerHTML = `
            <p>Games in ${getMonthName(
              selectedMonth
            )}/${selectedYear}: <strong>${filteredData.length}</strong></p>
    ...
    let totalCumulativeScore = cumulativeData.reduce(
      (sum, entry) => sum + entry.myScore,
      0
    );
    let avgTotalScore = (totalCumulativeScore / cumulativeData.length).toFixed(
      6
    );
    ...
    statsResults.innerHTML = `
        <p>You have played <strong>${
          filteredData.length
        }</strong> games in ${getMonthName(
      selectedMonth
    )},  ${selectedYear} ! </p>
        <p>Your average score for ${getMonthName(
          selectedMonth
        )},  ${selectedYear}: <strong>${avgMonthScore}</strong> !</p>
        <p>Your average score from Day 1 to ${getMonthName(
          selectedMonth
        )},  ${selectedYear}: <strong>${avgTotalScore}</strong> !</p>
    `;
  }
...
function displayLetterScoreStats(letter) {
  ...
  wordleWords.forEach((entry) => {
    if (entry.word.includes(letter)) {
      ...
      totalWords++;
      wordList.push(
        `<p>${entry.word} - Score: ${entry.myScore} (Date: ${entry.gameDate})</p>`
      );
    }
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  letterInput.addEventListener("input", () =>
    displayLetterScoreStats(letterInput.value)
  );
});
...
function displayLetterScoreStatsOne(letter) {
  if (letter.length !== 1) {
    document.getElementById("scoreDisplayOne").innerHTML =
  ...
  for (let i = 0; i <= 7; i++) {
    if (wordGroups[i].length > 0) {
      outputHTML += `<p><strong>Words with Score ${i}:</strong></p><p>${wordGroups[
        i
      ].join("<br>")}</p>`;
    }
  }
