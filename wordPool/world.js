...
const scoreStreaksWordsTitle = document.getElementById("scoreStreaksWordsTitle");
...
toggleButton.addEventListener("click", () => {
  ...
  // If hiding the main section, also hide the score average filters and streaks
  if (isVisible) {
...
function updateScoreStreaksDisplay() {
  ...
  if (streakData.maxLength === 0) {
    const modeText = mode === "exact" ? `score ${score}` : `scores below ${score}`;
  ...
  sortedLengths.forEach((length, lengthIndex) => {
    ...
    const rankLabel = lengthIndex === 0 ? "Longest" : 
                     lengthIndex === 1 ? "2nd Longest" :
                     lengthIndex === 2 ? "3rd Longest" :
    ...
    streaksOfThisLength.forEach((streak) => {
      ...
      if (mode === "below") {
        const totalScore = streak.reduce((sum, entry) => sum + entry.myScore, 0);
  ...
  document.querySelectorAll(".view-streak-button").forEach((button) => {
    button.addEventListener("click", (e) => {
      ...
      setTimeout(() => {
        scoreStreaksWordsContainer.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
      }, 100);
    });
  });
}
